...
{
    int numElements, offset,height, stripsize, myrank, i, j,k, iter ,startCol;
    ...
    //height,temp,myrank,numnodes

   // #pragma omp parallel for shared(orginalGrid,local_max_diff) private(startCol,iter,i,j,height,myrank) schedule (static, chunkSize)
    //height,temp,myrank,numnodes
   //works #pragma omp parallel for shared(orginalGrid,MAX_DIFF,local_max_diff,numThreads) private(startCol,iter,temp,i,j) schedule (static, chunkSize)

    for(iter=0;iter<=maxiters;iter++)
    {
    #pragma omp parallel for shared(orginalGrid,MAX_DIFF,numThreads,local_max_diff) private(i,j,temp) schedule (static, chunkSize)
...
#pragma omp parallel for shared(orginalGrid,MAX_DIFF,local_max_diff,numThreads) private(i,j,temp) schedule (static, chunkSize)
            ...
            //MPI_Barrier(MPI_COMM_WORLD);  -- Not required , was used to synchronize send and recv
        if(numnodes < 2)
        {
            // Dont do anything this is like sequential
        }
    ...
    if(myrank == 0)
    {
        endTime = MPI_Wtime();
        printf("\nResults are ::");
        printf("Number of MPI Ranks :: %d\t",numnodes);
        printf("Number of Threads :: %d\t",numThreads);
        printf("Execution time :: %0.3f\t",(endTime-startTime));
